define(['jquery','utils','template','uploadify','jcrop','form'],function($,utils,template,uploadify){
  // 导航栏选中高亮显示
  utils.setMenu('/course/course_add');
  // 获取csid
  var csId=utils.qs('cs_id');
  var flag=utils.qs('flag');
  // 获取课程封面数据
  $.ajax({
    type:'get',
    url:'/api/course/picture',
    data:{cs_id:csId},
    dataType:'json',
    success:function(data){
      console.log(data);
      if(flag){
        data.result.operate="课程编辑";
      }else{
         data.result.operate="课程添加";
      }
      var html=template('pictureTpl',data.result);
      $("#pictureInfo").html(html);
      // 获取图片
      var img=$(".preview img").eq(0);
      var nowCrop=null;
      // 处理图片上传
      $("#myfile").uploadify({
        width: 80,
        height: 'auto',
        itemTemplate:'<span></span>',
        buttonText:'选择图片',
        // 按钮样式
        buttonClass:'btn btn-success btn-sm',
        // swf文件地址
        swf:'/public/assets/uploadify/uploadify.swf',
        uploader:'/api/uploader/cover',
        // 上传图片名字
        fileObjName:'cs_cover_original',
        formData:{cs_id:csId},
        onUploadSuccess:function(a,b,c){
          var obj=JSON.parse(b.trim());
          $(".preview img").attr('src',obj.result.path);
           cropImage();
           $('#cropBtn').text('保存图片').attr('data-apple',true);
        }
      });

      // 处理封面裁切
        $("#cropBtn").click(function(){
          var apple=$(this).attr('data-apple');
          if(apple){
             $("#cropForm").ajaxSubmit({
                type:'post',
                url:'/api/course/update/picture',
                data:{cs_id:csId},
                dataType: 'json',
                success:function(data){
                  // console.log(data);
                  location.href='/course/lesson?cs_id='+data.result.cs_id;
                }
             })
          }else{
           // 第一次点击
           $(this).text('保存图片').attr('data-apple',true);
           cropImage()
          }
        });

        // 封装裁切图片功能方法
        function cropImage(){
          img.Jcrop({
            aspectRatio:2
          },function(){
          
            // 销毁之前的裁切实例
            nowCrop && nowCrop.destroy();
            nowCrop=this;

            // 获取图片的宽高
            var width=this.ui.stage.width;
            var height=this.ui.stage.height;
            // 计算裁切部分参数
            var x=0;
            var y=(height-width/2)/2;
            var w=width;
            var h=width/2;
            // 初始化默认参数
            var aInput=$("#cropForm").find('input');
            aInput.eq(0).val(x);
            aInput.eq(1).val(y);
            aInput.eq(2).val(w);
            aInput.eq(3).val(h);

            // 动态创建一个选区
            this.newSelection();
            this.setSelect([x,y,w,h]);
            // 初始化缩略预览图
             this.initComponent('Thumbnailer',{
              width:240,
              height:120,
              mythumb:'.thumb'
            });
             $(".jcrop-thumb").css({
              position:'absolute',
              top:0,
              left:0,
             });
             // 监控选区变化事件
             img.parent().on('cropstart cropmove cropend',function(a,b,c){
              var aInput=$("#cropForm").find('input');
                 aInput.eq(0).val(c.x);
                 aInput.eq(1).val(c.y);
                 aInput.eq(2).val(c.w);
                 aInput.eq(3).val(c.h);
             })
          })
        }
    }
  })
})
